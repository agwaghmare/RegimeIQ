...
Train/test split at 2022-01-01.
  - Pre-2022 (train): target = sigmoid-rescaled raw sum
...
def _sigmoid_rescale(raw):
    ...
    sig_0 = 1.0 / (1.0 + np.exp(-k * (0 - c)))
    sig_13 = 1.0 / (1.0 + np.exp(-k * (13 - c)))
...
def predict_scores(signals_df: pd.DataFrame) -> pd.Series:
    ...
    Falls back to sigmoid-rescaled raw sum if model is unavailable.
    """
    model = _load_model()
    if model is None:
        logger.warning("No model available — falling back to sigmoid rescaling")
...
def predict_score_single(signals: dict, raw_total_score: int) -> float:
    ...
    Returns float score on 0-10 scale.
    """
    ...
    # Flatten nested signals dict into a one-row DataFrame, then run through
    # _build_feature_matrix so interaction features are added — matching
    # the 42-feature matrix the model was trained on.
    flat: dict = {}
    ...
    try:
        X = _build_feature_matrix(row_df)
