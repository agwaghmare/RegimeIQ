...
@router.get(
    "/snapshot",
    summary="Full regime dashboard for any historical date",
)
def get_regime_snapshot(date: str = Query(..., description="Target date YYYY-MM-DD")):
    try:
...
@router.get(
    "/export",
    summary="Download all current regime data as a CSV file",
)
def export_regime_csv():
    ...
    fw = data.get("fedwatch", {}).get("next_3m", {})
    ...
    row: dict = {
        ...
        # breakdown
        "growth_score": breakdown.get("growth", {}).get("score"),
        "inflation_score": breakdown.get("inflation", {}).get("score"),
        "financial_score": breakdown.get("financial", {}).get("score"),
        "market_score": breakdown.get("market", {}).get("score"),
        # allocation
        "alloc_equities": alloc.get("equities"),
        "alloc_bonds": alloc.get("bonds"),
        "alloc_gold": alloc.get("gold"),
        # growth signals
        **{f"growth_{k}": v for k, v in sig.get("growth", {}).items() if k != "date"},
        # inflation signals
        **{f"inflation_{k}": v for k, v in sig.get("inflation", {}).items() if k != "date"},
        # financial signals
        **{f"financial_{k}": v for k, v in sig.get("financial", {}).items() if k != "date"},
        # market signals
        **{f"market_{k}": v for k, v in sig.get("market", {}).items() if k != "date"},
        # global macro
        **{f"macro_{k}": v for k, v in gm.items()},
        # fedwatch
        "fedwatch_cut": fw.get("cut"),
        "fedwatch_hold": fw.get("hold"),
        "fedwatch_hike": fw.get("hike"),
    }
    ...
    filename = f"regimeiq_export_{data.get('date', 'unknown')}.csv"
